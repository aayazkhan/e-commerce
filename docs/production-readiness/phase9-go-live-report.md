...
24. **Known risks** — Gateway routing was completed 2026-09-08 (real reverse proxy to all 24 services with edge routing, tested and cross-checked against every service's OpenAPI contract via `tests/contracts/validate-contracts.sh` — see [coverage-exceptions.md](../phase10/coverage-exceptions.md) and [final-test-report.md](../phase10/final-test-report.md) for detail); raw manifests have mixed historical conventions; production images/registry/cluster/IaC provider are not connected; live capacity and recovery evidence is absent.
