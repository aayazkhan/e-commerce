...
@Composable
fun LoginScreen(authApi: AuthApi, onLoggedIn: (accessToken: String) -> Unit) {
        ...
        scope.launch {
            when (val result = authApi.login(identifier.trim(), password)) {
                ...
                is ApiResult.NetworkError -> errorMessage =
                    "Couldn't reach the server. Is the backend running at localhost:8080?"
            }
            isSubmitting = false
        }
    }
    ...
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Column(
            ...
            Text("Seller Console", style = MaterialTheme.typography.headlineMedium)
            LabeledTextField("Email or phone", identifier, modifier = Modifier.fillMaxWidth()) { identifier = it }
