...
class SearchMigrationIntegrationTest {
    private val postgres = PostgreSQLContainer<Nothing>("postgres:16-alpine")
    @BeforeAll fun start() { postgres.start() }
    @Test fun `search migration applies`() { ServiceDatabase(ServiceDatabaseConfig(postgres.jdbcUrl, postgres.username, postgres.password), "classpath:db/migration").use { assertTrue(it.ping()) } }
}
