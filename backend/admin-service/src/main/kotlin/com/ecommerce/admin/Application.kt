...
internal class AdminRepositoryAdapter(private val delegate: AdminRepository) : AdminStore {
...
internal class AdminProxyAdapter(private val delegate: DownstreamHttpClient) : AdminProxy {
...
internal fun ApplicationConfig.required(p:String)=property(p).getString();internal fun parseKeys(v:String)=v.split(',').associate{it.substringBefore('=').trim() to it.substringAfter('=').trim()}.filterValues{it.isNotBlank()};@Serializable internal data class Health(val status:String,val component:String)
