...
fun Application.configureMediaRoutes(repository: MediaStore, storage: MediaBlobStore, verifier: HmacJwtAccessVerifier, maxBytes: Long, bucket: String, processor: MediaProcessor, scope: CoroutineScope) {
    ...
    routing {
        ...
        get("/api/v1/media/{mediaId}") { val asset = repository.find(call.parameters.requireValue("mediaId")) ?: throw ApiException(ErrorCode.NOT_FOUND, "Media asset not found.", 404); call.respond(asset.copy(variants = asset.variants.map { it.copy(url = "/api/v1/media/${asset.id}/variants/${it.id}/view") })) }
        ...
        // The bucket is Private (no public-read config, no provider billing verification needed
        // for that) -- these are the stable, non-expiring URLs worth storing anywhere an image
        // is referenced (e.g. a product's media list); each hit mints a fresh short-lived
        // presigned GET and redirects to it, so the reference itself never goes stale. Public
        // (no auth) on purpose: product imagery needs to load in a plain <img> tag.
        get("/api/v1/media/{mediaId}/view") { val asset = repository.find(call.parameters.requireValue("mediaId")) ?: throw ApiException(ErrorCode.NOT_FOUND, "Media asset not found.", 404); call.respondRedirect(storage.presignGet(asset.objectKey), permanent = false) }
        get("/api/v1/media/{mediaId}/variants/{variantId}/view") { val asset = repository.find(call.parameters.requireValue("mediaId")) ?: throw ApiException(ErrorCode.NOT_FOUND, "Media asset not found.", 404); val variant = asset.variants.find { it.id == call.parameters.requireValue("variantId") } ?: throw ApiException(ErrorCode.NOT_FOUND, "Media variant not found.", 404); call.respondRedirect(storage.presignGet(variant.objectKey), permanent = false) }
    }
}
