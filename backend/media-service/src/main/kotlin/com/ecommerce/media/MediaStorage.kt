...
class MediaStorage internal constructor(
    ...
    private companion object {
        fun s3Client(region: String, endpoint: String?): S3Client = S3Client.builder().region(Region.of(region)).apply { if (!endpoint.isNullOrBlank()) endpointOverride(URI.create(endpoint)) }.build()
        fun s3Presigner(region: String, endpoint: String?): S3Presigner = S3Presigner.builder().region(Region.of(region)).apply { if (!endpoint.isNullOrBlank()) endpointOverride(URI.create(endpoint)) }.build()
    }
}
