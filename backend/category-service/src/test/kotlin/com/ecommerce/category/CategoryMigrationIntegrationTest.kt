...
class CategoryMigrationIntegrationTest {
    ...
    @Test fun `category migration applies`() { ServiceDatabase(ServiceDatabaseConfig(postgres.jdbcUrl, postgres.username, postgres.password), "classpath:db/migration").use { assertTrue(it.ping()) } }
}
