...
class CategoryModuleBootIntegrationTest {
    companion object {
    ...
    fun `real module wiring boots against a live database, redis, and kafka broker`() = testApplication {
        environment {
            config = MapApplicationConfig(
                "category.database.url" to postgres.jdbcUrl,
                "category.database.username" to postgres.username,
                "category.database.password" to postgres.password,
                "category.jwt.issuer" to "issuer",
                "category.jwt.audience" to "audience",
                "category.jwt.keys" to "kid-1=secret",
                "category.kafka.bootstrapServers" to kafka.bootstrapServers,
                "category.kafka.topic" to "category.events",
                "category.kafka.tenantId" to "tenant-test",
                "category.redis.url" to "redis://${redis.host}:${redis.getMappedPort(6379)}",
            )
        }
        application { module() }
