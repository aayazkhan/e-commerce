...
class PricingModuleBootIntegrationTest {
    companion object {
    ...
    fun `real module wiring boots against a live database, redis, and kafka broker`() = testApplication {
        environment {
            config = MapApplicationConfig(
                "pricing.database.url" to postgres.jdbcUrl,
                "pricing.database.username" to postgres.username,
                "pricing.database.password" to postgres.password,
                "pricing.jwt.issuer" to "issuer",
                "pricing.jwt.audience" to "audience",
                "pricing.jwt.keys" to "kid-1=secret",
                "pricing.kafka.bootstrapServers" to kafka.bootstrapServers,
                "pricing.kafka.topic" to "pricing.events",
                "pricing.kafka.tenantId" to "tenant-test",
                "pricing.redis.url" to "redis://${redis.host}:${redis.getMappedPort(6379)}",
            )
        }
        application { module() }
