...
fun Application.module() {
    ...
    val providers=mapOf<PaymentProviderName,PaymentProvider>(PaymentProviderName.COD to CodPaymentProvider(),PaymentProviderName.HTTP to HttpPaymentProvider(config.required("payment.provider.baseUrl"),config.required("payment.provider.apiKey"),config.required("payment.provider.webhookSecret"),json))
...
fun Application.configurePaymentRoutes(store: PaymentStore, verifier: HmacJwtAccessVerifier, internalToken: String, webhookVerifier: PaymentWebhookVerifier, json: Json) {
    routing {
