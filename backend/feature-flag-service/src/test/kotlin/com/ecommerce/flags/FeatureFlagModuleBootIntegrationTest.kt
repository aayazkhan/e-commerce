...
class FeatureFlagModuleBootIntegrationTest {
    companion object {
    ...
    fun `real module wiring boots against a live database, redis, and kafka broker`() = testApplication {
        environment {
            config = MapApplicationConfig(
                "flags.database.url" to postgres.jdbcUrl,
                "flags.database.username" to postgres.username,
                "flags.database.password" to postgres.password,
                "flags.database.maximumPoolSize" to "5",
                "flags.database.connectionTimeoutMillis" to "5000",
                "flags.jwt.issuer" to "issuer",
                "flags.jwt.audience" to "audience",
                "flags.jwt.keys" to "kid-1=secret",
                "flags.kafka.bootstrapServers" to kafka.bootstrapServers,
                "flags.kafka.topic" to "flags.events",
                "flags.kafka.tenantId" to "tenant-test",
                "flags.redis.url" to "redis://${redis.host}:${redis.getMappedPort(6379)}",
            )
        }
        application { module() }
