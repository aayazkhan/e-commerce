...
class CatalogMigrationIntegrationTest {
    ...
    @Test fun `catalog migration applies`() { ServiceDatabase(ServiceDatabaseConfig(postgres.jdbcUrl, postgres.username, postgres.password), "classpath:db/migration").use { assertTrue(it.ping()) } }
}
