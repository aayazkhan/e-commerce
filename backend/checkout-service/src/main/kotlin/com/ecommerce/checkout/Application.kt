...
interface CheckoutDependencies {
    ...
    suspend fun createPayment(userId: String, token: String, checkoutId: String, orderId: String, totals: CheckoutTotals, request: CheckoutRequest, details: CheckoutDetails): PaymentWire
...
private val checkoutSagaLogger = LoggerFactory.getLogger("CheckoutSaga")
...
internal class CheckoutClients private constructor(
    ...
    override suspend fun createPayment(userId:String,token:String,checkoutId:String,orderId:String,totals:CheckoutTotals,request:CheckoutRequest,details:CheckoutDetails):PaymentWire{val body=PaymentEnvelopeWire(userId,PaymentRequestWire(orderId,totals.totalMinor,totals.currency,request.paymentProvider.uppercase(),request.paymentMethodToken,customerName=details.shipping.recipientName,customerPhone=details.shipping.phone,checkoutId=checkoutId));return decode(payment.post("/api/v1/internal/payments",json.encodeToString(body),mapOf("X-Internal-Service-Token" to token,"Idempotency-Key" to "payment-$checkoutId")))}
    ...
    override suspend fun transitionOrder(orderId:String,status:String,token:String){require2xx(order.post("/api/v1/internal/orders/$orderId/status",json.encodeToString(StatusWire(status)),mapOf("X-Internal-Service-Token" to token)))}
    ...
    private inline fun <reified T>decode(response:InternalHttpResponse):T{require2xx(response);return json.decodeFromString(response.body)}
    private fun require2xx(response:InternalHttpResponse){if(response.statusCode !in 200..299)throw ApiException(ErrorCode.DEPENDENCY_UNAVAILABLE,"Checkout dependency rejected the request.",503,true)}
}
...
@Serializable private data class PaymentRequestWire(val orderId:String,val amountMinor:Long,val currency:String,val provider:String,val paymentMethodToken:String,val customerName:String?=null,val customerEmail:String?=null,val customerPhone:String?=null,val checkoutId:String?=null)
