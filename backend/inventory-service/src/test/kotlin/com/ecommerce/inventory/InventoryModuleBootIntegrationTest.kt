...
class InventoryModuleBootIntegrationTest {
    companion object {
    ...
    fun `real module wiring boots against a live database, redis, and kafka broker`() = testApplication {
        environment {
            config = MapApplicationConfig(
                "inventory.database.url" to postgres.jdbcUrl,
                "inventory.database.username" to postgres.username,
                "inventory.database.password" to postgres.password,
                "inventory.database.maximumPoolSize" to "5",
                "inventory.database.connectionTimeoutMillis" to "5000",
                "inventory.expirationIntervalSeconds" to "3600",
                "inventory.internalToken" to "internal-test-token",
                "inventory.jwt.issuer" to "issuer",
                "inventory.jwt.audience" to "audience",
                "inventory.jwt.keys" to "kid-1=secret",
                "inventory.kafka.bootstrapServers" to kafka.bootstrapServers,
                "inventory.kafka.topic" to "inventory.events",
                "inventory.kafka.tenantId" to "tenant-test",
                "inventory.redis.url" to "redis://${redis.host}:${redis.getMappedPort(6379)}",
            )
        }
        application { module() }
